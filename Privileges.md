...
* Can approve other users' watches and blacklists on GitHub. Simply add a comment on the auto-generated PR containing the command `!!/approve`, and metasmoke will handle the rest. In order for this to work, you will need a GitHub account that is known to us. Ping an admin and they will set you up.  
  *[Note to admins: In addition to adding the Code Admin role to the user's MS account, the GitHub user must be added to the [Other Awesome People (OAPs) team on GitHub](https://github.com/orgs/Charcoal-SE/teams/oaps/members) (only visible to Charcoal-SE members). If the user is not already a member of the [Charcoal-SE GitHub organization](https://github.com/Charcoal-SE), they will need to be invited to join Charcoal-SE and accept the invitation; you can [invite them and add them to OAPs at the same time](https://github.com/orgs/Charcoal-SE/teams/oaps/members). After they are a member of Charcoal-SE, they need to be added to [.pullapprove.yml](https://github.com/Charcoal-SE/SmokeDetector/blob/master/.pullapprove.yml), and you need to press the 'sync everything' button [here](https://pullapprove.com/Charcoal-SE/SmokeDetector/settings/) (near the bottom of the page).]*
