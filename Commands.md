...
- `!!/report <post URL 1> [<post URL 2> [...]]` — Makes SmokeDetector report a specific post/multiple specific posts in Charcoal HQ and other applicable rooms. The originator of each post will be added to the blacklist. Maximum 5 at a time. Additionally, the post will be added to the database on Metasmoke, just like all other posts.
...
- `!!/unwatch <regex>` - Removed a previously-added regular expression from the watch list. Only code admins can use this command; it will only accept an exact match to a regular expression in the watchlist.
